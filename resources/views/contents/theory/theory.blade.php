						            ...
						            <form role="form" method="POST" action="{{ url('admin/media/store/') }}" enctype="multipart/form-data">
						            	@csrf
						              	<div class="box-body">
						                	...
						                	<div class="form-group">
						                  		{{-- <label for="">FILE</label> --}}
						                  		<input type="hidden" class="form-control" id="" name="from" value="theory">
						                	</div>
						              	</div>
						              	<!-- /.box-body -->
		                ...
		                <tbody>
		                	...
		                	@foreach ($files as $key => $file)
				                <tr>
				                  	...
				                  	@elseif(in_array($file->file_type, ["jpg", "jpeg", "png"]))
				                  		<td><img src="{{ $path.'/images/'.$file->file_name}}" width="30%"></td>
				                  	@else
				                  		<td>
				                  			@php($url = "https://docs.google.com/gview?url=".$path.'/files/'.$file->file_name."&embedded=true")
				                  			<iframe class="doc" src="{{ $url }}"></iframe>
				                  	@endif
				                  	<td>
				                  		<a href="{{ url('admin/theory/media/delete/'.$file->id )}}" class="btn btn-default btn-xs"><i class="fa fa-trash"></i> Delete</a>
				                  	</td>
				                </tr>
			                @endforeach
		             	</tbody>
